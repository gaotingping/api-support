...
public class ScannerUtils{
	...
	public static List<Class<?>> getServiceEntry(List<String> packages,Class<? extends Annotation> annotationType) throws IOException, ClassNotFoundException {

		List<Class<?>> classSet = new ArrayList<>();
		
		TypeFilter typeFilter = new AnnotationTypeFilter(annotationType, false);
			...
			for (String pkg : packages) {
				...
				for (Resource resource : resources) {
					if (resource.isReadable()) {
						...
						if (matchesEntityTypeFilter(reader, readerFactory,typeFilter)) {
